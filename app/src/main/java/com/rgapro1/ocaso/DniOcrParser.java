...
public final class DniOcrParser {
    ...
    public static Result parse(String raw){
        Result r=new Result(); String text=normalizeOcr(raw==null?"":raw); String[] lines=text.split("\\R");
        for(int i=0;i<lines.length;i++){String line=clean(lines[i]);if(line.isEmpty())continue;
            if(isLabel(line,"APELLIDOS","APELLIDO"))r.surname=collectSurname(lines,i);
            else if(isLabel(line,"NOMBRE")){String v=valueAfter(line,"NOMBRE");if(v.isEmpty())v=nextSimpleValue(lines,i);if(isHumanName(v))r.name=v;}
            ...
            else if(line.contains("DOMICILIO")){String v=valueAfter(line,"DOMICILIO");if(v.isEmpty())v=nextSimpleValue(lines,i);r.address=v;}
            else if(line.contains("LUGAR DE NACIMIENTO")){String v=valueAfter(line,"LUGAR DE NACIMIENTO");if(v.isEmpty())v=nextSimpleValue(lines,i);r.birthPlace=v;}
            else if(line.contains("NUM SOPORTE")||line.contains("Nº SOPORTE")||line.contains("N° SOPORTE")){String v=valueAfter(line,"NUM SOPORTE","Nº SOPORTE","N° SOPORTE");if(v.isEmpty())v=nextSimpleValue(lines,i);r.supportNumber=v;}
            else if(line.contains("EMISION")||line.contains("EMISIÓN")){r.issueDate=firstDate(line);if(r.issueDate.isEmpty())r.issueDate=firstDate(nextSimpleValue(lines,i));}
            else if(line.contains("VALIDEZ")||line.contains("CADUCIDAD")){r.validityDate=firstDate(line);if(r.validityDate.isEmpty())r.validityDate=firstDate(nextSimpleValue(lines,i));}
        }
        r.dni=findDni(text); r.birthDate=findBirthDate(text,lines); String mrz=collectMrz(lines);r.mrz=mrz;
        if(!mrz.isEmpty()){
        ...
        if(r.name.isEmpty()){Matcher m=Pattern.compile("NOMBRE\\s*[:.-]?\\s*([A-ZÁÉÍÓÚÑ]{2,}(?:\\s+[A-ZÁÉÍÓÚÑ]{2,}){0,2})").matcher(text);if(m.find()&&isHumanName(m.group(1)))r.name=m.group(1).trim();}
    ...
    private static String collectSurname(String[] lines,int i){String first=valueAfter(lines[i],"APELLIDOS","APELLIDO");StringBuilder b=new StringBuilder();if(isHumanName(first))b.append(first);for(int j=i+1;j<Math.min(lines.length,i+5);j++){String x=clean(lines[j]);if(x.isEmpty()||isFieldLabel(x))break;if(isHumanName(x)){if(b.length()>0)b.append(' ');b.append(x);}else break;}return cleanSurname(b.toString());}
    private static String cleanSurname(String s){return clean(s).replaceAll("\\s+"," ").trim();}
    private static boolean isFieldLabel(String s){return s.startsWith("NOMBRE")||s.startsWith("SEXO")||s.contains("NACIONALIDAD")||s.contains("NACIMIENTO")||s.contains("DOMICILIO")||s.contains("SOPORTE")||s.contains("VALIDEZ")||s.contains("CADUCIDAD")||s.contains("EMISION")||s.startsWith("FIRMA")||s.startsWith("NUMERO")||s.startsWith("REINO")||s.startsWith("DOCUMENTO")||s.startsWith("NACIONAL");}
    private static boolean isHumanName(String s){return s!=null&&s.matches("[A-ZÁÉÍÓÚÑ]{2,}(?:[ -][A-ZÁÉÍÓÚÑ]{2,})*")&&!isFieldLabel(s)&&!s.contains("ESPAÑA")&&!s.equals("IDENTIDAD")&&!s.equals("CARD");}
    ...
    private static String firstDate(String s){if(s==null)return "";Matcher m=Pattern.compile("\\b\\d{2}\\s*[ /.-]\\s*\\d{2}\\s*[ /.-]\\s*\\d{4}\\b").matcher(s);return m.find()?normalizeDate(m.group()):"";}
    ...
    private static String collectMrz(String[] lines){StringBuilder b=new StringBuilder();for(String line:lines){String c=line.toUpperCase(Locale.ROOT).replace(" ","");int lt=0;for(int k=0;k<c.length();k++)if(c.charAt(k)=='<')lt++;if(c.length()>=20&&(c.contains("<<")||c.startsWith("IDESP")||(c.length()>=28&&lt>=2))){if(b.length()>0)b.append('\n');b.append(c.replaceAll("[^A-Z0-9<]",""));}}return b.toString().trim();}
