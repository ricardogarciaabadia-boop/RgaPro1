...
public final class DniOcrParser {
    ...
    public static Result parse(String raw){
        ...
        if(!mrz.isEmpty()){
            ...
            if(dates.find()){r.birthDate=mrzDate(dates.group(1));r.sex=dates.group(2);r.validityDate=mrzDate(dates.group(3));}
            if(r.nationality.isEmpty()&&mrz.contains("ESP"))r.nationality="ESP";
        }
        if(r.name.isEmpty()||r.surname.isEmpty()){
            for(int i=0;i<lines.length;i++){
                String l=clean(lines[i]);
                if(r.name.isEmpty()&&l.startsWith("NOMBRE")){String v=valueAfter(l,"NOMBRE");if(v.isEmpty()&&i+1<lines.length)v=clean(lines[i+1]);if(isHumanName(v))r.name=v;}
        ...
        int score=0; if(!r.dni.isEmpty())score+=25; if(!r.name.isEmpty())score+=20; if(!r.surname.isEmpty())score+=20; if(!r.birthDate.isEmpty())score+=15; if(!r.validityDate.isEmpty())score+=10; if(!r.nationality.isEmpty())score+=5; if(!r.sex.isEmpty())score+=5;
    ...
    private static String findDni(String text){
        Pattern labeled=Pattern.compile("(?:DNI|NIF|NIE)\\s*[:.-]?\\s*((?:[0-9]\\s*){8}|[XYZ]\\s*[0-9\\s]{7})\\s*([A-Z])\\b"); Matcher m=labeled.matcher(text); String fallback="";
        while(m.find()){String c=(m.group(1)+m.group(2)).replaceAll("\\s","");if(c.matches("\\d{8}[A-Z]")&&isValidDni(c))return c;if(c.matches("[XYZ]\\d{7}[A-Z]")&&isValidNie(c))return c;if(fallback.isEmpty())fallback=c;}
        m=Pattern.compile("(?<![A-Z0-9])((?:\\d\\s*){8})([A-Z])(?![A-Z0-9])").matcher(text); while(m.find()){String c=(m.group(1)+m.group(2)).replaceAll("\\s","");if(isValidDni(c))return c;if(fallback.isEmpty())fallback=c;}
        m=Pattern.compile("(?<![A-Z0-9])([XYZ]\\s*(?:\\d\\s*){7})([A-Z])(?![A-Z0-9])").matcher(text); while(m.find()){String c=(m.group(1)+m.group(2)).replaceAll("\\s","");if(isValidNie(c))return c;if(fallback.isEmpty())fallback=c;}
        return fallback;
    }
    private static String collectSurname(String[] lines,int i){String first=valueAfter(lines[i],"APELLIDOS","APELLIDO");StringBuilder b=new StringBuilder();if(isHumanName(first))b.append(first);for(int j=i+1;j<Math.min(lines.length,i+4);j++){String x=clean(lines[j]);if(x.isEmpty()||isFieldLabel(x))break;if(isHumanName(x)){if(b.length()>0)b.append(' ');b.append(x);}else break;}return b.toString().replaceAll("\\s+"," ").trim();}
