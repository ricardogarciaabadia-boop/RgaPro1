...
public class MainActivity extends Activity {
    ...
    private boolean matches(JSONObject p,String q){if(q==null||q.trim().isEmpty())return true;String n=q.toLowerCase(Locale.ROOT);for(String k:new String[]{"holder","holderDni","phone","email","address","type","number","expiry"})if(p.optString(k).toLowerCase(Locale.ROOT).contains(n))return true;return false;}
    ...
    private void security(){page("🔐 Seguridad");content.addView(text("Acceso protegido por clave. Cada usuario tiene su cartera separada.",16,TEXT,false));content.addView(text("Compartir datos requiere confirmación explícita.",16,TEXT,false));}
}
