...
public class RgaProActivity extends Activity {
    ...
    private String findPolicyNumber(String raw){if(raw==null)return"";Matcher m=Pattern.compile("(?i)(?:N[º°O]\\s*)?(?:NÚMERO DE P[ÓO]LIZA|NUMERO DE POLIZA|P[ÓO]LIZA|POLIZA)\\s*[:#-]?\\s*([A-Z0-9./_-]{4,})").matcher(raw);if(m.find())return m.group(1).trim();m=Pattern.compile("(?i)(?:PRODUCTO|SUBP[ÓO]LIZA|Nº DE P[ÓO]LIZA)[^0-9A-Z]{0,20}([0-9]{5,14})").matcher(raw);return m.find()?m.group(1):"";}
    private String findPolicyType(String raw){String u=(raw==null?"":raw).toUpperCase(Locale.ROOT);if(u.contains("ASISTENCIA FAMILIAR"))return"Asistencia Familiar";if(u.contains("DECESOS"))return"Decesos";if(u.contains("VIDA"))return"Vida";if(u.contains("AHORRO"))return"Ahorro";if(u.contains("HOGAR"))return"Hogar";if(u.contains("COMUNIDAD")||u.contains("COMUNIDADES"))return"Comunidades";if(u.contains("RESPONSABILIDAD CIVIL"))return"Responsabilidad Civil";if(u.contains("AUTO")||u.contains("AUTOMOVIL")||u.contains("AUTOMÓVIL"))return"Auto";return"Póliza";}
    private String findCompany(String raw){String u=(raw==null?"":raw).toUpperCase(Locale.ROOT);if(u.contains("OCASO"))return"Ocaso";return"";}
    ...
    private Bitmap enhance(Bitmap src){Bitmap out=Bitmap.createBitmap(src.getWidth(),src.getHeight(),Bitmap.Config.ARGB_8888);android.graphics.Canvas c=new android.graphics.Canvas(out);android.graphics.Paint p=new android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG);android.graphics.ColorMatrix cm=new android.graphics.ColorMatrix(new float[]{1.22f,0,0,0,6,0,1.22f,0,0,6,0,0,1.22f,0,6,0,0,0,1,0});p.setColorFilter(new android.graphics.ColorMatrixColorFilter(cm));c.drawBitmap(src,0,0,p);return out;}
