...
'''
    # add a security page entry with registration action
    marker='    private void security(){'
    idx=s.find(marker)
    if idx>=0:
        end=s.find('\n    }',idx)+6
        # don't replace existing security method; add registration button by inserting just before its closing brace
        ...
        # only inject if security method has a page/content body
        close=s.find('\n    }',idx)
        if close>=0:s=s[:close]+sec+s[close:]
...
if 'shareSecureClient(p);' not in s:
    ...
    idx=s.find(marker)
    if idx>=0:
        # inject immediately after method opening brace
        openb=s.find('{',idx)
