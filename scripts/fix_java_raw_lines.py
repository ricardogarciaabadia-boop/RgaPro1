...
for i, line in enumerate(lines):
    ...
    # A lambda used in an assignment must terminate with ';' after its closing brace.
...
if changed:
    JAVA.write_text('\n'.join(lines) + ('\n' if s.endswith('\n') else ''), encoding='utf-8')
    print('Malformed Java raw-line, lambda termination, and escaped-punctuation normalization fixed')
else:
